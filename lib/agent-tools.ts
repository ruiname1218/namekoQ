...
import { Output, generateText, tool, type LanguageModel } from "ai";
import { z } from "zod";
...
export function resolveModelProfile(tier: ModelTier): ModelProfile {
  if (tier === "pro") {
    return {
      tier,
      label: "GPT-5.5",
...
function addDeepSeekCompatibilityBody(init: RequestInit | undefined) {
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({
        ...body,
        thinking: isRecord(body.thinking) ? body.thinking : { type: "disabled" },
      }),
    };
  } catch {
    return init;
...
function schemaDescription(schema: z.ZodType<unknown>): string {
  ...
  if (schema === openQasmPreparationSchema) {
    return JSON.stringify({ extractionCode: "string", notes: ["string"] });
  }
  return "{}";
}
...
const simulationInputSchema = z.object({
  ...
  purpose: z.string().describe("このシミュレーションで確認したいことを短く書く"),
});
...
const verdictSchema = z.object({
  ...
  mismatches: z
    .array(
      z.object({
        aspect: z.string().describe("ズレている観点"),
        expected: z.string().describe("ユーザーが意図したと思われる内容"),
        actual: z.string().describe("コード/結果が実際に行った内容"),
      }),
    )
    .describe("具体的なズレ"),
  suggestions: z.array(z.string()).describe("aligned=falseの場合の修正案"),
  summary: z.string().describe("1〜2文の日本語サマリー"),
});

export function createVerifyIntentTool(
  profile: ModelProfile,
  getStepsUsed: () => number,
) {
  return tool({
    ...
    inputSchema: z.object({
      userRequest: z.string().describe("ユーザーの元の要望をそのままコピーする"),
      interpretation: z.string().describe("要望をどう解釈したか1〜2文で説明する"),
      plan: PlanSchema.describe("request_planで受理されたplanオブジェクトをそのまま渡す"),
      generatedCode: z.string().describe("対応するsimulation toolへ渡した実際のコード全文"),
      result: z.unknown().describe("対応するsimulation toolのparsed結果"),
    }),
    execute: async ({ userRequest, interpretation, plan, generatedCode, result }) => {
      const started = Date.now();
      ...
      try {
        ...
        return { ...verdict, durationMs: Date.now() - started };
      } catch (err) {
...
const openQasmPreparationSchema = z.object({
  ...
  notes: z.array(z.string()).describe("抽出コードが表現する範囲と制約。なければ空配列"),
});
...
export function createConvertToOpenQasmTool(profile: ModelProfile) {
  return tool({
    description: [
    ...
    execute: async ({ framework, generatedCode, plan, result }) => {
      ...
      try {
        ...
        const fallbackOpenqasm =
          typeof parsed.fallback_openqasm === "string" ? parsed.fallback_openqasm : undefined;
        const openqasmError =
          typeof parsed.openqasm_error === "string" ? parsed.openqasm_error : undefined;
        const openqasmVersion =
          typeof parsed.openqasm_version === "string" ? parsed.openqasm_version : undefined;
        ...
        return {
          ...
          convertedFrameworkCodes: openqasm
            ? createFrameworkConversionCodes({ sourceFramework: framework, openqasm, openqasmVersion })
            : {},
...
export function createAgentTools(
  ...
  return {
    ...
    verify_intent_alignment: createVerifyIntentTool(profile, getStepsUsed),
    convert_to_openqasm: createConvertToOpenQasmTool(profile),
  };
}
...
function createFrameworkConversionCodes({
  ...
  return Object.fromEntries(
    targets
      .filter((t) => t !== sourceFramework)
      .map((t) => [t, createFrameworkConversionCode({ target: t, openqasm, openqasmVersion })]),
  );
}
...
function normalizeOpenQasmForEditor(openqasm: string): string {
  ...
  return openqasm
    ...
    .replace(/[δΔ]/g, "delta").replace(/[θΘ]/g, "theta").replace(/[λΛ]/g, "lambda")
    .replace(/[μΜ]/g, "mu").replace(/[πΠ]/g, "pi").replace(/[φΦ]/g, "phi")
    .replace(/[ψΨ]/g, "psi").replace(/[ωΩ]/g, "omega").replace(/[^\x00-\x7F]/g, "_");
}
